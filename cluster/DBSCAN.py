...
# 画最终聚类效果图
def show_result(class_list, raw_data, name):
    colors = [
    ...
    for node, class_ in enumerate(class_list):
        if class_ != 'border' and class_ != -1:
            plt.scatter(x=raw_data[node, 0], y=raw_data[node, 1], c=colors[use_color[class_]], s=50, marker='o', alpha=0.9)
        elif class_ == 'border':
            plt.scatter(x=raw_data[node, 0], y=raw_data[node, 1], c='g', s=200, marker='^',
                        alpha=0.8)
        else:
            plt.scatter(x=raw_data[node, 0], y=raw_data[node, 1], c='b', s=200, marker='+',
                        alpha=0.8)
    plt.title('The Result Of Cluster')
    # plt.show()
    plt.savefig('../result/DBSCAN_Result_' + name + '.jpg' )


def main(raw_data, name, M, t):
    ...
    show_result(class_list, raw_data, name)

if __name__ == '__main__':
    M = 3    #  M一般设为数据的维度+1
    t = 2    #  t值限制截断距离（即epsilon的值）
    name = 'Jain'
    data_path = r'../data/Jain.txt'
    raw_data = np.loadtxt(data_path, delimiter='	', usecols=[0, 1])
    main(raw_data, name, M, t)
