...
#  通过公式np.sum(np.exp(-(node / dc) ** 2))衡量一个点的密度（连续型）/ 通过数距离小于dc的点的个数来衡量一个点的密度（离散型）
def count_or_continous_density(distance, dc, method = None):
...
def show_nodes_for_chosing_mainly_leaders(density, detal_ls, name):
    ...
    plt.scatter(x=range(len(detal_ls)), y=-np.sort(-gamma), c='#000000', marker='o', s=-np.sort(-gamma) * 100)
    plt.xlabel('data_num')
    plt.ylabel('gamma')
    plt.title('CFSDP: Guarantee The Leader')
    # plt.show()
    plt.savefig('../result/CFSDP_Guarantee_' + name + '.jpg')
    return gamma
...
# 画最终聚类效果图
def show_result(new_class, norm_data, chose_list, name):
    colors = [
    ...
    for node, class_ in enumerate(new_class):
        ...
        if node in chose_list:
            plt.scatter(x=norm_data[node, 0], y=norm_data[node, 1], marker='+', s=200, c='b', alpha=0.8)
        else:
            plt.scatter(x=norm_data[node, 0], y=norm_data[node, 1], c=colors[leader_color[class_]], s=50, marker='o', alpha=0.9)
    plt.title('CFSDP: The Result Of Cluster')
    # plt.show()
    plt.savefig('../result/CFSDP_Result_' + name + '.jpg')


# 画detal图和原始数据图
def show_optionmal(den, det, v, name):
    colors = [
              '#FF0000', '#FFA500', '#FFFF00', '#00FF00', '#228B22',
              '#0000FF', '#FF1493', '#EE82EE', '#000000', '#FFA500',
              '#00FF00', '#006400', '#00FFFF', '#0000FF', '#FFFACD',
    ...
    # ax1 = plt.subplot(121)
    # for i in range(len(v)):
    #     plt.scatter(x=den[i], y=det[i], c='#000000', marker='o', s=15)
    # plt.xlabel('density')
    # plt.ylabel('detal')
    ...
    # ax2 = plt.subplot(122)
    for j in range(len(v)):
        plt.scatter(x=v[j, 0], y=v[j, 1], marker='o', c=colors[int(v[j, 2])], s=8)
    plt.xlabel('axis_x')
    plt.ylabel('axis_y')
    plt.title('Dataset')
    # plt.sca(ax2)
    # plt.show()
    plt.savefig('../result/CFSDP_ChoseAndDataset_' + name + '.jpg' )


def main(input_x, name, t):
    norm_data = input_x
    distance = caldistance(norm_data)  # 制作任意两点之间的距离矩阵
    dc = chose_dc(distance, t)  # 根据t选择合适的dc
    density = count_or_continous_density(distance, dc)  # 统计每点的密度
    detal_ls, closest_leader = node_detal(density, distance)  # 统计每点的直接上司
    show_optionmal(density, detal_ls, norm_data, name)  # 展示原始数据集并为选择司令个数提供参考
    scores = show_nodes_for_chosing_mainly_leaders(density, detal_ls, name)  # 进一步确认选择的司令个数是否正确
    leaders_num = int(input('input clusters num\n'))  # 根据以上的数据处理,输入最终选择的司令数目
    chose_list = np.argsort(-scores)[: leaders_num]  # 选择聚类中心
    print('mianly leaders:', chose_list)
    new_class = clustering(closest_leader, chose_list)  # 确定各点的最终归属（哪个司令）
    show_result(new_class, norm_data, chose_list, name)  # 展示结果


if __name__ == '__main__':
    t = 2
    # name = 'Spiral'
    # data_path = r'../data/Spiral.txt'
    # name = 'Flame'
    # data_path = r'../data/Flame.txt'
    # name = 'Aggregation'
    # data_path = r'../data/Aggregation.txt'
    name = 'Jain'
    data_path = r'../data/Jain.txt'
    raw_data = np.loadtxt(data_path, delimiter='	', usecols=[0, 1, 2])
    main(raw_data, name, t)
